...
class CartControllerTest {
    ...
    private Logininfo cantLoginUser; // ログインできないユーザー
    ...
    void setUp() {
        ...
        Logininfo loginuser = new Logininfo("名前", "sample@loginsuccess.com", "successpassword",  "東京都東京市東京町1-1-1");
        Logininfo cantlLoginUser 
                = new Logininfo("名前", "sample@loginfail.com","failpassword","東京都東京市東京町1-1-1");
        // 他のメソッドはテストごとに when で設定
    }
   // === GET /api/cart ===
    @Nested
    @DisplayName("GET /api/mypage")
    class GetMypageTests {
    @Test
        @DisplayName("存在するemailの場合、レスポンスボディにinfo の JSONを返す。")
        void getmypage_WhenUserExists_ShouldReturnLoginInfotWithStatusOk() throws Exception {
            ...
            mockMvc.perform(get("/api/mypage")
                    ...
                    .andExpect(jsonPath("$.email".isNotNull()))
                    // 全フィールドを検証
